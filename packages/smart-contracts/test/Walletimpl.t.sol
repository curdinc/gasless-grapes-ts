...
contract WalletTest is Test {
    ...
    function testSingleContractCallSigned() public {
        ...
        Transaction memory transaction = Transaction(1000000, address(counter), false, 0, abi.encodeWithSignature("increment()"));
        ...
        (success, result) = (alice_ggwallet).call{value: 0, 
                            gas: 3000000}(abi.encodeWithSignature("execute((uint256,address,bool,uint256,bytes),uint256,bytes)",
                            transaction, nonce, alice_signature));
        // END OF EXECUTE // 
